...
# Explicit declaration of all state variables
__all__ = [
...
def initialize_state():
    return {
        ...
        "extracted_columns": [],  # Add this line
        "data_available": False,
        "selected_statistic": "mean"
    }
...
def extract_range_graph_data(file_path, selected_line, selected_variable_enum):
    """Extract range graph data with depth as z"""
    try:
        model = OrcFxAPI.Model(file_path)
        ...
        if not isinstance(selected_variable_enum, OrcFxAPI.RangeGraphVariable):
            raise ValueError(f"Invalid variable enum: {selected_variable_enum}")
            
        rg = line.RangeGraph(selected_variable_enum, period)
        ...
        print(f"Extracted {len(df)} data points for line '{selected_line}', variable '{selected_variable_enum}'")
        return df
    except Exception as e:
        print(f"Error extracting data for line '{selected_line}': {e}")
        import traceback
        traceback.print_exc()
...
def process_selected_data(state, action, payload):
    ...
    try:
        ...
        if not result_df.empty:
            ...
            state.extracted_columns = list(result_df.columns)  # Add this line
